        ...
        for line in file:
            ...
            if float(line[2])>50.0 and max([ int(i) for i in re.split(r'(\d+)',line[-1]) if len(i)>0 and i.isdigit()])/float(line[3])>=0.40:
                if line[0]!=line[1]:
                    ...
                    if overlap>=1.0:
                        bhits+=[(line[0],pbidict[line[0]],line[1],pbidict[line[1]],line[-2])]
...
def readgff(f):
    ...
    for line in filename:
        ...
        if line[0][:8]=='scaffold' and line[2]=='gene':
            if line[0] not in pbi:
                pbi[line[0]]=[re.split(r'[=;\s]',line[8])[1]]
            else:
                pbi[line[0]].append(re.split(r'[=;\s]',line[8])[1])
    return pbi 
