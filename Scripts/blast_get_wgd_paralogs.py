...
def ProcessCLI(args):
    ...
    scaf,pbidict=readgff(args[2])[:2]
    enout=includeScaffTopsl(pslout,pbidict,args[3])
    sssd=getSSS(enout,pbidict)[1]
    anotssd=sssCheck(args[4],pbidict)
    protprotHits=get_rbsssh(enout,pbidict,sssd)
    candidates=findCandidates(protprotHits)
    bestpairs=getbesthits(candidates)
    rbbh,oneways=getrbbsh(bestpairs)
    hits=rbbh+oneways
    allpairsfromSSmethod=list(itertools.chain(*useSStogetrbbh(protprotHits,sssd,scaf)))

    ## Generate some output files 
    printSSS(anotssd,'Gold_Standard_Scaff_Scaff_pairs.txt')
    printSSS(sssd,'Predicted_Scaff_Scaff_pairs.txt')
    printHits(hits,'Predicted_hits.txt')
    printHits(allpairsfromSSmethod,'HitsPredictedwithinSSapproach.txt')
    
    
## From scaffolds-scaffold pairs whose score exceeds some threshold, find the protein paralogs between them
def useSStogetrbbh(pphits,ssd2,scaf):
    allscaf=[]
    for s in ssd2.keys():
        seen=[]
        ssh=[]
        alls2=sorted(ssd2[s], key=ssd2[s].get)
        for s2 in alls2:
            if float(ssd2[s][s2])< 10.0:
                lss=[]
                for p in scaf[s]:
                    for p2 in scaf[s2]:
                        if p in pphits.keys() and p2 in pphits[p].keys():
                            lss+=[p,p2,pphits[p][p2]]
                            seen.append(p)
            ssh+=[[lss]]
        allscaf+=[[ssh]]
    return allscaf 

## Separate reciprocal best hits from one-way best hits 
...
## For each protein choose the best paralog from its paralog candidates bades scaffold-scaffold score and bit score
def getbesthits(candidates):
    ...
    for k in candidates.keys():
        ...
        for v in candidates[k].keys():
            if float(candidates[k][v][2])<bestsss:
                bestbit=float(candidates[k][v][0])
                bestsss=float(candidates[k][v][2])                
                bpairs[(k,v)]=candidates[k][v]
            elif float(candidates[k][v][2])==bestsss:
                if float(candidates[k][v][0])>bestbit:
                    bestbit=float(candidates[k][v][0])
                    bestsss=float(candidates[k][v][2])
                    bpairs[(k,v)]=candidates[k][v]
    return bpairs 

## Find paralog candidates for each protein 
...
## Create a dictionary of protein P1 and all the other proteins P2 and bit score,evalue and sss
#{P1:{P1_1:(bscore,eval,sss),P1_2:(bscore,eval,sss),...},P2{P2_1:(bscore,eval,sss),P2_2:(bscore,eval,sss),..}}
def get_rbsssh(en_pslout,pbidict,sssd2):
    ...
    for line in en_pslout:
        ...
        if jfidentl>0.5 and float(line[4])>40.0 and ov/min(float(line[5]),float(line[6]))>=0.12:
            if line[0]!=line[2] and line[1]!=line[3]:
                ...
                if line[0] not in ppevsssd.keys():
                    ...
                    ppevsssd[line[0]][line[2]]=(float(line[-3]),float(line[-2]),sc)
                elif line[0] in ppevsssd.keys() and line[2] in ppevsssd[line[0]].keys():
                    if float(ppevsssd[line[0]][line[2]][0])> float(line[-3]):
                        continue 
                    else:
                        ppevsssd[line[0]][line[2]]=(float(line[-3]),float(line[-2]),sc)
    return  ppevsssd

## Get our first predicted Scaffold-Scaffold pair score (SSS)
## <<Criteria cutoffs can be optimized for each genome tested>>
   ...
   # testfile=open('testfile','a')
    ...
    for k1 in sca_List_sca.keys():
        ...
        for k2 in sca_List_sca[k1].keys():
         ...
         #   testfile.write(k1+ ' '+ k2+' '+ str(score) +'\n')
    # we return the list of all scaffold-scaffold pairs and sorted dictionary of each scaffold and the scaffolds it hits 
...
## Get enriched output : add scaffolds information for each protein in the output
...
## From the paralogs from McGrath, create scaffold-scaffold scores  
...
#Get the scaffold-scaffold pair(s1,s2) score from the scaffolds-scaffolds dictionary  
def findSSS(s1,s2,sssd2):
    try:
        ...
        if x==y: 
            ...
            return 1337 ## flag cases where scaffolds score (s1,S2) does not match (s2,s1)
    except KeyError:
        return 200.0 ## default all one way scaffolds hits to score 200
...
# Match gene names between files 
def GtoT(pbid):
    pbid=list(pbid)
...
## Create a dictionary of paralogs from the McGrath paralog file 
    ...
    for line in filename:
        if len(line)>0:
            ...
            if GtoT(line[0]) not in d.keys() and "." not in GtoT(line[0]) and "." not in GtoT(line[1]):
                ...
                elif "." in GtoT(line[0]) and "." not in GtoT(line[1]):
                    d1.append(GtoT(line[1]))
    return d,d1

##Create a dictioray of protein -protein hits given an output of reported protein hits 
def getpphits(nf):
    inf=readfile(nf)
    pp={}
    for line in inf:
        line=line.split()        
        if line[0] not in pp.keys():
            pp[line[0]]={}
            
            pp[line[0]][line[1]]=tuple([float(i) for i in re.split(r'[(,)\s]',''.join(line[2:])) if len(i)>0])
            
        else:
            pp[line[0]][line[1]]=tuple([float(i) for i in re.split(r'[(,)\s]',''.join(line[2:])) if len(i)>0])
    return pp
## write out the scaffold-scaffold pairs score
def printSSS(d,out):
    o_file=open(out,'w')
    for k in d.keys():
        if type(d[k])is dict:
    ...
    o_file.close()
## write out the protein-protein hits score 
...
## Separate reciprocal hits from one-way hits when the input is given as a dictionary
##{P1:P1_paralog}                        
def checkrbbh(d):
    bestrbbh={}
    onewayhits={}
