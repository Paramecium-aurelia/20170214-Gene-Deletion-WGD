...
def ProcessCLI(args):
    ...
    #print len(sssd),len(anotssd)
    printSSS(anotssd,'ohnologs')
    printSSS(sssd,'bi_scaffolds') 
    ...
    hits=rbbh+oneways
    printHits(hits,'bi_bothits')
    #print len(rbbh),len(oneways)
...
## Using the dictionary input (k1,k2):(bsc,ev,sss)
...
def get_rbsssh(en_pslout,pbidict,sssd2):
    ...
    for line in en_pslout:
        ...
        #if float(line[4])>40.0 and ov/min(float(line[5]),float(line[6]))>=0.10:
        #again, we need to work with the jfidentl
        if jfidentl>0.5 and pident>30.0:
...
## Get ScaScaScore (SSS)::(sca_sca_list,Sca1(key):list of (sca1,sca2,sss),sssList)
def getSSS(en_pslout,pbidict):
    sssd=collections.OrderedDict()
    sca_List_sca=collections.OrderedDict()
    sssList=[]
    #en_pslout=en_pslout.split('\n')
    #with open(en_pslout,'r') as file:
    for line in en_pslout:
        line=line.rstrip().split()
        jfident=(float(line[-4])/(min(float(line[5]),float(line[6]))))
        ...
        #if jfidentl>0.4 and ov/min(float(line[5]),float(line[6]))>=0.12:#JF wants to know what happens when we skip the 12%!!!
                                    #I think we might also want to replace the 12% with a number of pident
        #if jfident>0.4 and ov/min(float(line[5]),float(line[6]))>=0.12:
        if jgindentl>0.6 and pident>40.0
            if line[0]!=line[2] and line[1]!=line[3]: 
...
def sssCheck(infile,pbi):
    ...
    for line in filename:
        if len(line)>0 and 'hno' not in line and "." not in line:
            ...
            if pbi[GtoT(line[0])] not in sca_List_sca.keys():# and "." not in GtoT(line[0]) and "." not in GtoT(line[1]):
